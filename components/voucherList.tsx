...
export default function VoucherList({ vouchers, onSelect, selectedEntries }:VoucherListProps) {
  const [selectAll, setSelectAll] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);
  const [vouchersPerPage] = useState(20);
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead>Sale Entry Date</TableHead> {/* Added Sale Entry Date */}
          </TableRow>
        </TableHeader>
        <TableBody>
          {currentVouchers.map((voucher, index) => (
            <TableRow key={index}>
              ...
              <TableCell>
                {new Date(voucher.InvoiceEntryDate).toLocaleString()}
              </TableCell>
              {/* Added Invoice Entry Date */}
              ...
              <TableCell>
                {new Date(voucher.SaleEntryDate).toLocaleString()}
              </TableCell>
